...
def generate_n_colors(n):
    """
    Generate a list of n colors distributed in equal intervals.

    Args:
    - n (int): Number of colors.

    Returns:
    - list: List of n colors in RGB format.
    """
    colors = []
    for i in range(n):
        hue = i / n
        lightness = 0.5
        saturation = 0.9  # can adjust to get more pastel or more vibrant colors
        rgb = colorsys.hls_to_rgb(hue, lightness, saturation)
        rgb_scaled = tuple(int(val * 255) for val in rgb)
        colors.append('#{:02x}{:02x}{:02x}'.format(*rgb_scaled))
    return colors


def name_to_rgb(color_name):
    try:
        # Convert color name to RGB float values (between 0 and 1)
        rgb_floats = mcolors.to_rgba(color_name)[:3]
        
        # Convert RGB float values to integer values (between 0 and 255)
        rgb_ints = tuple([int(255 * x) for x in rgb_floats])
        return rgb_ints
    except ValueError:
        return "Nombres de color inválidos."
...
class DrillData:
    ...
    def plot_3d(self, feature):
        ...
        if self.feature_palette:
            cat_palette = self.feature_palette[feature]
        else:
            cat_palette = None
        ...
        if dtype == "categoric":
            categories = points[feature].unique()
            colors = dict(zip(categories, generate_n_colors(len(categories))))
            for value in categories:
                df = points[points[feature] == value]
                line_color = cat_palette[value] if cat_palette and value in cat_palette else f"rgb{name_to_rgb(colors[value])}"
                fig.add_trace(go.Scatter3d(x=df["X"], y=df["Y"], z=df["Z"], name=value,
                                           ...
                                           line=dict(width=7, color=line_color)
                                          )
                             )
        elif dtype == "numeric":
